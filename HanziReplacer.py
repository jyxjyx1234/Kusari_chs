...
class HanziReplacer(object):
    def __init__(self) -> None:
        ...
        self.tempdict={}
        self.hanzidict={}
        self.allchar=list(self.charlist)
    ...
    def ChangeFNTFont(self,ori_path,out_path):
        l = len(self.allchar)
        ori_FNT=list(open_file_b(ori_path))

        tihuandict={}
        for i in range(len(self.source_chars)):#target为正常显示的文字，source为sjis支持的临时替代汉字
            tihuandict[self.target_chars[i]]=self.source_chars[i]

        save_json("Hanzitihuandict.json",tihuandict)
        ...
        for char in self.allchar:
            ...
            if char in tihuandict:
                char_ = tihuandict[char]
                o = getoffset(char_)
            else:
                o = getoffset(char)
            new_bmp = process_result(draw_character(char,'wenquanyi.ttf'))
            ori_FNT[o:o+24*24]=new_bmp
        ori_FNT = bytes(ori_FNT)
        save_file_b(out_path,ori_FNT)
